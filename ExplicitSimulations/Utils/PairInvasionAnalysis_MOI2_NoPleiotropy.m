...
files = dir('Data/NoPleiotropy/Mutant*.mat');
...
parfor i = 1:length(files)
    ...
    fname = files(i).name
      % Define regex pattern to extract z1 and z2
    pattern = 'z1=([-+]?[0-9]*\.?[0-9]+),z2=([-+]?[0-9]*\.?[0-9]+)';
    ...
    if ~isempty(tokens)
        z1 = str2double(tokens{1}{1});
        z2 = str2double(tokens{1}{2});
        datatable(i,:) = array2table([z1 z2 r]);
        
    else
        warning('Skipping file (no z1/z2 found): %s', fname);
    end
end
